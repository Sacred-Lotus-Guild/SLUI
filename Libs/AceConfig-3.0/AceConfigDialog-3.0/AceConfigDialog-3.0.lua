		...
		if not hidden then
			if v.type == "group" then
				...
				elseif v.type == "input" then
					...
					-- Disable Accept button for read-only multiline inputs
					if v.multiline and control.DisableButton then
						local setFunc = GetOptionsMemberValue("set",v, options, path, appName)
						-- Disable button if set function is nil or if disableButton property is set
						if not setFunc or v.disableButton then
